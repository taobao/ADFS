...
public class DistributedServer implements DistributedInvocable {
  ...
  /**
   * format invocation needs to wait until other data invocations and getData invocation have completed,
   * and then block new request until format invocation has completed.
   */
  ReentrantReadWriteLockExtension formatLocker = new ReentrantReadWriteLockExtension();
  /**
  ...
  protected DistributedServer(Configuration conf) throws IOException {
  ...
  boolean initialize() throws IOException {
    ...
    threadLocalInvocation.get().setCallerAddress(InetAddress.getByName(Utilities.getHost(serverName)).getHostAddress());
  ...
  static String getServerName(Configuration conf) throws IOException {
    ...
    if (serverName == null || serverName.split(":").length != 2) {
      ...
      String hostname = inetAddressList.isEmpty() ? "localhost" : inetAddressList.get(0).getHostAddress();
      if (hostname.startsWith("127")) hostname = "localhost";
      serverName = hostname + ":40000";
    }
  ...
  static void configLogger(Configuration conf) throws IOException {
    ...
    Utilities.setConfDefaultValue(conf, appenderName + ".File", "logs/distributed-server-"
        + conf.get("distributed.server.name").replace(':', '-') + ".log");
  ...
  synchronized public ServerStatuses updateServers() {
    ...
    synchronized (statusLock) {
      serverType = newServerStatusOfThisServer.type;
      ...
      if (!newServerStatusOfThisServer.isTypeApplied()
        ...
        register(serverName, data.getDataVersion(), serverType);
      }
    }
  ...
  void restoreDistributedDataLocker(DistributedData newData, String masterName) throws IOException {
    log(Level.INFO, " start to restore distributed data locker from master=", masterName);
    DistributedLocker locker = (DistributedLocker) newData.getElementToTransfer("distributed.data.locker");
    if (locker == null) throw new IOException("no distributed locker is found in new data");
    data.getDataLocker().set(locker);
    log(Level.DEBUG, " restore ", locker);
    log(Level.INFO, " succeed in restoring ", locker.getSize(), " distributed data locker from master=", masterName);
  }
  ...
  void restoreAllFromMasterServer(ServerStatus master) {
    try {
      ...
      restoreDistributedDataLocker(dataFromMaster, master.name);
      data.open();
      // some fail operation or global lock service will cause current version large than max data version
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (conf.getBoolean("distributed.data.format", false)) formatServer(conf);
    else if (conf.getBoolean("distributed.server.stop", false)) stopServer(conf);
    else startServer(conf, true);
  }

  public static void formatServer(Configuration conf) throws IOException {
    stopServer(conf);
    startServer(conf);
  }
  ...
  public static void startServer(Configuration conf, boolean inMainThread) throws IOException {
    if (inMainThread) {
      startServer(conf);
    } else {
      class DistributedServerThread extends Thread {
        ...
        public void run() {
          try {
            DistributedServer.startServer(conf);
          } catch (Throwable t) {
            Utilities.logError(logger, "server exits abnormally", t);
          }
        }
      }
      new DistributedServerThread(conf).start();
    }
  }

  private static void startServer(Configuration conf) throws IOException {
    try {
      new DistributedServer(conf).waitUntilStop();
    } catch (Throwable t) {
      throw new IOException(t);
    }
  }
  ...
  public Object read(Invocation dataInvocation) throws IOException {
    if (!serverType.is(ServerType.MASTER, ServerType.SLAVE)) {
      String message = serverName + "|" + serverType + " refuse to do read call " + dataInvocation;
      throw new DistributedException(true, new IOException(message));
    }

    data.invoke(dataInvocation);
    long debugElapsedTime = conf.getLong("distributed.server.debug.elapsed.time", 0);
    log(dataInvocation.getElapsedTime() >= debugElapsedTime ? Level.DEBUG : Level.TRACE, " do read call ",
        dataInvocation);
    return dataInvocation.getResult();
  }

  public Object write(Invocation dataInvocation) throws IOException {
    ...
    if (!serverTypeSnapshot.is(ServerType.MASTER, ServerType.SLAVE, ServerType.UNDER_RESTORE)) {
      String message = serverName + "|" + serverTypeSnapshot + " refuse to do write call " + dataInvocation;
      throw new DistributedException(true, new IOException(message));
    }
    ...
    if (serverTypeSnapshot.is(ServerType.MASTER)) {
      try {
        data.invoke(dataInvocation);
        if (DistributedResult.needRetry(dataInvocation.getResult())) return dataInvocation.getResult();
        long debugElapsedTime = conf.getLong("distributed.server.debug.elapsed.time", 0);
        log(dataInvocation.getElapsedTime() >= debugElapsedTime ? Level.DEBUG : Level.TRACE, " do write call ",
            dataInvocation);
        if (serverTypeSnapshot.is(ServerType.MASTER)) {
          writeBackupServers(dataInvocation, bakupServersSnapshot);
          if (!serverType.is(ServerType.MASTER)) {
            log(Level.WARN, " master has changed ", serverStatuses);
            throw new DistributedException(true, new IOException("master changed when write result to slave"));
          }
        }
        return dataInvocation.getResult();
      } catch (Throwable t) {
        // when data invocation throw an exception, maybe, some date have been written
        Object resultForServerInDistributedException = DistributedException.getDistributedExceptionResult(t);
        if (resultForServerInDistributedException != null && serverTypeSnapshot.is(ServerType.MASTER)) {
          dataInvocation.setResult(resultForServerInDistributedException);
          log(Level.WARN, " do write call ", dataInvocation, " with exception=", t);
          writeBackupServers(dataInvocation, bakupServersSnapshot);
          if (!serverType.is(ServerType.MASTER)) {
            log(Level.WARN, " master has changed ", serverStatuses);
            throw new DistributedException(true, new IOException(
                "master has changed when write DistributedException result to slave"));
          }
        }
        throw new IOException(t);
      }
    } else {
      if (!ServerType.SLAVE.equals(serverType) || conf.getBoolean("distributed.server.write.async", false)) {
  ...
  public void writeBackupServers(Invocation dataInvocation, List<ServerStatus> bakupServers) {
    if (bakupServers == null || dataInvocation.getResult() == null) return;
    for (ServerStatus serverStatus : bakupServers) {
      try {
        long startTime = System.currentTimeMillis();
        dataInvocation.setResult(serverStatus.proxy.invoke(getDataInvocation(dataInvocation)));
        dataInvocation.setElapsedTime(System.currentTimeMillis() - startTime);
  ...
  public Object dataInvoke(Invocation invocation) throws IOException {
    Class<?> invocationType = DistributedData.getDistributedInvocationType(invocation.getMethod());
    Lock formatLock = invocation.getMethodName().equals("format") ? formatLocker.writeLock() : formatLocker.readLock();
    try {
      ...
      // format invocation need to wait all other in flight invocations
      formatLock.lock();
      getDataLocker.readLock().lock();

      long startTime = System.currentTimeMillis();
      if (invocationType.equals(DistributedRead.class)) invocation.setResult(read(invocation));
      else if (invocationType.equals(DistributedWrite.class)) invocation.setResult(write(invocation));
      else throw new IOException("no or wrong operation annotation for " + invocation.getMethod().toGenericString());
      ...
      if (DistributedException.isNeedRestore(t)) {
        ...
        if (serverType.is(ServerType.MASTER) && serverStatuses.getSlaves(false).isEmpty()) {
          try {
            closeData();
            openData();
            if (invocationType.equals(DistributedRead.class)) return read(invocation);
          } catch (Throwable t2) {
  ...
  public static Invocation getCurrentInvocation() {
    return threadLocalInvocation.get();
  }
  ...
  public Object invoke(Invocation invocation) throws IOException {
    ...
    try {
      ...
      invocation.setCallerAddress(RPC.Server.getRemoteAddress());
      if ("dataInvoke".equals(invocation.getMethodName())) {
        ...
        dataInvocation.setCallerOfCallerAddress(invocation.getCallerOfCallerAddress());
