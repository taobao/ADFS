...
public class DistributedFunctionTest {
  ...
  public void clusterTest() throws Exception {
    ...
    // lock same key by two threads, and make sure master can stop and retry the second lock
    assertThat(client.lock(null, Long.MAX_VALUE, Long.MAX_VALUE, "A") != null, is(true));
    ++version;
    class MyRunnable implements Runnable {
      ExampleProtocol client = null;
      DistributedLock result = null;

      MyRunnable(ExampleProtocol client) {
        this.client = client;
      }

      public void run() {
        try {
          result = client.lock(null, Long.MAX_VALUE, Long.MAX_VALUE, "A");
          client.unlock(null, "A");
        } catch (Throwable t) {
          t.printStackTrace();
        }
      }
    ...
    assertThat(myRunnable.result == null, is(true));
    assertThat(client.unlock(null, "A") != null, is(true));
    ++version;
    Thread.sleep(200);
    assertThat(myRunnable.result != null, is(true));
    ++version;
  ...
  public void lockAndUnlock(ExampleProtocol client) throws Exception {
    assertThat(client.lock(null, Long.MAX_VALUE, Long.MAX_VALUE, 0) != null, is(true));
    assertThat(client.tryLock(null, Long.MAX_VALUE, 0) == null, is(true));
    assertThat(client.unlock(null, 0) != null, is(true));

    assertThat(client.lock(null, 100, Long.MAX_VALUE, 0) != null, is(true));
    Thread.sleep(200);
    assertThat(client.lock(null, Long.MAX_VALUE, Long.MAX_VALUE, 0) != null, is(true));
    assertThat(client.tryLock(null, Long.MAX_VALUE, 0) == null, is(true));
    assertThat(client.unlock(null, 0) != null, is(true));
