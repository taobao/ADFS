...
public class AppendOnly {
	...
	private static String logPath = "append.log";
	private static String appendPath = "/create";
	...
	private static void runTest() throws IOException {
		...
		String parmInfo = "operation:append_only;"+"threadNum:"+threadNum+
				";appendPath:"+appendPath+";fileNumPerThread:"+fileNumPerThread+";interval:"+interval;
		output.printParamInfo(parmInfo);
		for(int i =0; i<threadNum; i++)
		{
			threads[i] = new Thread(new AppendThread(i, fs, fileNumPerThread,
										appendPath, fire, over, output));
			threads[i].start();
		}
		try {
			fire.countDown();
	...
	private static void parseArgs(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-fileNumPerThread")) {
				...
				checkArgs(i + 1, args.length);
				logPath = args[++i];
			} else if (args[i].equals("-appendPath")) {
				checkArgs(i + 1, args.length);
				appendPath = args[++i];
	...
	private static void displayUsage() {
		String usage =
			      ...
			      "\t-appendPath <the path of appending file dir. default is /create. " + "This is not mandatory>\n" +
