 ...
 */
public class TestMNFSInputChecker extends TestCase {
  ...
  public void testFSInputCheckerDiffNN() throws Exception {
    Configuration conf = new Configuration();
    conf.setLong("dfs.block.size", BLOCK_SIZE);
    conf.setInt("io.bytes.per.checksum", BYTES_PER_SUM);
    conf.set("fs.hdfs.impl","org.apache.hadoop.hdfs.ChecksumDistributedFileSystem");
    ...
    cluster.waitActive();
    cluster.waitDatanodeDie();
    ChecksumFileSystem fileSys = (ChecksumFileSystem)cluster.getFileSystem(1);
    try {
      testChecker(fileSys, true);
      testChecker(fileSys, false);
      testSeekAndRead(fileSys);
