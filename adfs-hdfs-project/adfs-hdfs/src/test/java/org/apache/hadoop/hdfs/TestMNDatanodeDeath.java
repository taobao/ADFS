 ...
 */
public class TestMNDatanodeDeath extends TestCase {
  {
  ...
  //mode1 means nn1 get no dn and nn2 get all dn,client connect to nn1
  static final int mode1=1;
  //mode2 means nn1 get one dn and nn2 get the rest,client connect to nn1
  ...
  static private void writeFile(FSDataOutputStream stm, long seed) throws IOException {
    byte[] buffer = SNAppendTestUtil.randomBytes(seed, fileSize);
  ...
  static private void checkFile(FileSystem fileSys, Path name, int repl,
    ...
    while (!done) {
      ...
      for (int idx = 0; idx < locations.length; idx++) {
        //if (locations[idx].getHosts().length < repl) { // TODO make this check back
        if (locations[idx].getHosts().length < 1) {
         if (attempt > 100) {
    ...
    final byte[] expected = SNAppendTestUtil.randomBytes(seed, fileSize);
  ...
  class Modify extends Thread {
    ...
    Modify(Configuration conf, MiniMNDFSCluster cluster) {
      running = true;
    ...
    public void run() {
      ...
      while (running) {
        ...
        for (int i = 0; i < replication - 1; i++) {
          ...
          int victim = SNAppendTestUtil.nextInt(numDatanodes);
          try {
   ...
   * dies.
   */
  private void complexTest(int mode) throws IOException {
    ...
    String nameNodePort="0,0";
    conf.set("dfs.namenode.port.list", nameNodePort);
    MiniMNDFSCluster cluster = null;
    if( mode == mode1){
      cluster=new MiniMNDFSCluster(conf, numDatanodes, 1, true, null);
      cluster.waitActive();
    }else if( mode == mode2){
      cluster=new MiniMNDFSCluster(conf, numDatanodes-1, 1, true, null);
      cluster.waitActive();
    ...
    try {
      ...
      for (int i = 0; i < numThreads; i++) {
        workload[i] = new Workload(SNAppendTestUtil.nextLong(), fs, i, numberOfFiles, replication, 0);
        workload[i].start();
      }
   ...
   * close the file.
   */
  private void simpleTest(int datanodeToKill,int mode) throws IOException {
    Configuration conf = new Configuration();
    conf.setInt("heartbeat.recheck.interval", 2000);
    conf.setInt("datanodemap.update.interval", 3000);
    ...
    MiniMNDFSCluster cluster = null;
    if( mode == mode1){
      cluster=new MiniMNDFSCluster(conf, myMaxNodes, 1, true, null);
      cluster.waitActive();
      cluster.waitDatanodeDie();
    }else if( mode == mode2){
      cluster=new MiniMNDFSCluster(conf, myMaxNodes-1, 1, true, null);
      cluster.waitActive();
      cluster.startDataNodes(conf, 1, 0, true, null, null, null);
      cluster.waitDatanodeDie();
    }
    ...
    try {
      ...
      final long myseed = SNAppendTestUtil.nextLong();
      byte[] buffer = SNAppendTestUtil.randomBytes(myseed, fileSize);
      int mid = fileSize/4;
      stm.write(buffer, 0, mid);
      ...
      if (targets == null) {
        int victim = SNAppendTestUtil.nextInt(myMaxNodes);
  ...
  public void testSimple0() throws IOException {simpleTest(0,mode1);}

  public void testSimple1() throws IOException {simpleTest(1,mode1);}

  public void testSimple2() throws IOException {simpleTest(2,mode1);}
  
  public void testSimple3() throws IOException {simpleTest(0,mode2);}

  public void testSimple4() throws IOException {simpleTest(1,mode2);}

  public void testSimple5() throws IOException {simpleTest(2,mode2);}

//  public void testComplex0() throws IOException {complexTest(mode1);}
  
//  public void testComplex1() throws IOException {complexTest(mode2);}
