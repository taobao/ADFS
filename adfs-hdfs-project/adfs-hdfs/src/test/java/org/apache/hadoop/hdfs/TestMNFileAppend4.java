...
import org.apache.hadoop.hdfs.protocol.FSConstants;
import static org.apache.hadoop.hdfs.SNAppendTestUtil.loseLeases;
 ...
 */
public class TestMNFileAppend4 extends TestCase {
  static final Log LOG = LogFactory.getLog(TestMNFileAppend4.class);
  ...
  public void setUp() throws Exception {
    ...
    conf.setInt("heartbeat.recheck.interval", 50000);
...
//    conf.set("dfs.replication.pending.timeout.sec", Integer.toString(5));
  ...
  private void createFile(FileSystem whichfs, String filename, 
    ...
    SNAppendTestUtil.write(stm, 0, (int)fileSize);
  }
  ...
  private void recoverFile(final FileSystem fs) throws Exception {
    ...
    SNAppendTestUtil.recoverFile(cluster, fs, file1);
  ...
  private void waitForBlockReplication(FileSystem whichfs, String filename, 
    ...
    while (true) {
      ...
      BlockLocation[] bl = whichfs.getFileBlockLocations(whichfs.getFileStatus(file1), 0, BLOCK_SIZE);
      if(bl.length == 0) {
        replOk = false;
      }
  ...
  private void checkFile(FileSystem whichfs, long fileSize) throws Exception {
    ...
    SNAppendTestUtil.check(whichfs, file1, fileSize);
  }
  ...
  private void corruptDataNode(int dnNumber, CorruptionType type) throws Exception {
    ...
    //System.setProperty("test.build.data", "/home/hxq/hadoop-new/NN-refactor-split-ns-blocksmap/build/test/data");
  ...
  public void xxxtestAppendSyncBbwClusterRestart() throws Exception {
    LOG.info("START");
    cluster = new MiniMNDFSCluster(conf, 1, true, null);
    cluster.waitDatanodeDie();
    FileSystem fs1 = cluster.getFileSystemCToFreeNamenode(-1);
    FileSystem fs2 = null;
    try {
      ...
      fs2 = cluster.getFileSystemCToFreeNamenode(-1);
  ...
  public void xxxtestAppendSync2XBbwClusterRestart() throws Exception {
    LOG.info("START");
    cluster = new MiniMNDFSCluster(conf, 1, true, null);
    cluster.waitDatanodeDie();
  ...
  public void xtestAppendSyncBlockPlusBbw() throws Exception {
    LOG.info("START");
    cluster = new MiniMNDFSCluster(conf, 1, 0, true, null);
    cluster.waitActive();
    cluster.waitDatanodeDie();
  ...
  public void xxxtestAppendSyncReplication0() throws Exception {
    replicationTest(0);
  }
  public void xxxtestAppendSyncReplication1() throws Exception {
    replicationTest(1);
  }
  public void xxxtestAppendSyncReplication2() throws Exception {
    replicationTest(2);
  }
  ...
  void replicationTest(int badDN) throws Exception {
    ...
    try {
      ...
      SNAppendTestUtil.write(stm, 0, halfBlock);
      stm.sync();
      ...
      SNAppendTestUtil.write(stm, halfBlock, (int)BLOCK_SIZE/4);
      stm.sync();
      ...
      LOG.info("STOPPED first instance of the cluster");
      cluster = new MiniMNDFSCluster(conf, 3, false, null, false);
      cluster.getNameNode(0).getNamesystem().stallReplicationWork();
      cluster.waitActive();
      fs1 = cluster.getFileSystemCToFreeNamenode(-1);
      LOG.info("START second instance.");
      ...
      cluster.getNameNode(0).getNamesystem().restartReplicationWork();
  ...
  // we test different datanodes restarting to exercise 
  // the start, middle, & end of the DFSOutputStream pipeline
  public void xxxtestAppendSyncChecksum0() throws Exception {
    checksumTest(0);
  }
  public void xxxtestAppendSyncChecksum1() throws Exception {
    checksumTest(1);
  }
  public void xxxtestAppendSyncChecksum2() throws Exception {
    checksumTest(2);
  }
  ...
  void checksumTest(int goodDN) throws Exception {
    ...
    try {
      ...
      // stop the cluster
      cluster.getNameNode(0).setSafeMode(SafeModeAction.SAFEMODE_ENTER);
      cluster.shutdown(false);
      fs1.close();
      LOG.info("STOPPED first instance of the cluster");
  ...
  public void testRecoverFinalizedBlock() throws Throwable {
    ...
    try {
      cluster.waitActive();
      cluster.waitDatanodeDie();
      NameNode preSpyNN = cluster.getNameNode(0);
      ...
      // write 1/2 block
      SNAppendTestUtil.write(stm, 0, 4096);
  ...
  public void testCompleteOtherLeaseHoldersFile() throws Throwable {
    ...
    try {
      ...
      SNAppendTestUtil.write(appenderStream, 0, 4096);
  ...
  public void testDatanodeFailsToCommit() throws Throwable {
    ...
    cluster.waitDatanodeDie();
    FileSystem fs1 = cluster.getFileSystemCToFreeNamenode(-1);
    FileSystem fs2 = SNAppendTestUtil.createHdfsWithDifferentUsername(fs1.getConf());
    try {
  ...
  public void testBBWCleanupOnStartup() throws Throwable {
    ...
    try {
      ...
      // close one of the datanodes
      MiniMNDFSCluster.DataNodeProperties dnprops = cluster.stopDataNode(0);
      ...
      List<File> bbwFilesAfterRestart = getBBWFiles(dataDirs);
      while(bbwFilesAfterRestart.size()!=0)
      {
    	  cluster.waitDatanodeDie();
   ...
   */
  public void xtestRecoveryOnBlockBoundary() throws Throwable {
    LOG.info("START");
    cluster = new MiniMNDFSCluster(conf, 1, true, null);
    cluster.waitActive();
    cluster.waitDatanodeDie();
    FileSystem fs1 = cluster.getFileSystemCToFreeNamenode(-1);
    final FileSystem fs2 = SNAppendTestUtil.createHdfsWithDifferentUsername(fs1.getConf());
    ...
    try {
      ...
      SNAppendTestUtil.write(stm, 0, (int) (BLOCK_SIZE));
      stm.sync();
   ...
   */
  public void xxxtestAppendFileRace() throws Throwable {
    LOG.info("START");
    cluster = new MiniMNDFSCluster(conf, 1, true, null);
    cluster.waitActive();
    cluster.waitDatanodeDie();
    final FileSystem fs1 = cluster.getFileSystemCToFreeNamenode(-1);
   ...
   */
  public void xxxtestTruncatedPrimaryDN() throws Exception {
    LOG.info("START");
   ...
   * journal
   */
  public void xxxtestHalfLengthPrimaryDN() throws Exception {
    LOG.info("START");
  ...
  private void runDNRestartCorruptType(CorruptionType corrupt) throws Exception {
    ...
    try {
      ...
      SNAppendTestUtil.write(stm, 0, 1024);
      stm.sync();
      loseLeases(fs1);
      ...
      MiniMNDFSCluster.DataNodeProperties badDN = cluster.stopDataNode(0);
  ...
  public void xxxtestFullClusterPowerLoss() throws Exception {
    ...
    try {
      ...
      // close the DNs
      MiniMNDFSCluster.DataNodeProperties badDN = cluster.stopDataNode(0);
      MiniMNDFSCluster.DataNodeProperties badDN2 = cluster.stopDataNode(0); // what was 1 is now 0
      assertNotNull(badDN);
  ...
  public void testNotPrematurelyComplete() throws Exception {
    ...
    try {
      ...
      stm = fs1.append(file1);
      LOG.info("======== Writing");
      SNAppendTestUtil.write(stm, 0, halfBlock/2);
  ...
  public void testNotPrematurelyCompleteWithFailure() throws Exception {
    ...
    try {
      ...
      stm = fs1.append(file1);
      LOG.info("======== Writing");
      SNAppendTestUtil.write(stm, 0, halfBlock/4);
  ...
  public void testNotPrematurelyCompleteWithFailureNotReopened() throws Exception {
    ...
    try {
      ...
      LOG.info("======== Writing");
      SNAppendTestUtil.write(stm, 0, 1024*1024);
