 ...
 */
public class TestSNFileAppend2 {
  ...
  static Log LOG = LogFactory.getLog(TestSNFileAppend2.class);
  ...
  private void initBuffer(int size) {
    ...
    fileContents = SNAppendTestUtil.randomBytes(seed, size);
  }
  ...
  public void testSimpleAppend() throws IOException {
    ...
    try {
      ...
      { // test append permission.
        ...
        try {
          out = fs.create(foo);
          int len = 10 + SNAppendTestUtil.nextInt(100);
  ...
  class Workload extends Thread {
    ...
    Workload(MiniMNDFSCluster cluster, int threadIndex) {
      id = threadIndex;
    ...
    public void run() {
      ...
      for (int i = 0; i < numAppendsPerThread && err.get() == null; i++) {
        ...
        synchronized (testFiles) {
          ...
          int index = SNAppendTestUtil.nextInt(testFiles.size());
        ...
        try {
          ...
          sizeToAppend = SNAppendTestUtil.nextInt(left);
/*          //sizeToAppend = 128;
          if (fileSize - len < 128) {
            sizeToAppend = (int)(fileSize - len) / 3;
            if (sizeToAppend <= 0) {
              sizeToAppend = 1;
            }
          }*/
          ...
          while (fs.getFileStatus(testfile).getLen() != (len + sizeToAppend)) {
            try {
              ...
              assertTrue("Timed out waiting for len " + (len + sizeToAppend) +
                ...
                System.currentTimeMillis() - startWaitTime < 600000);
  ...
  public void testComplexAppend() throws Throwable {
    ...
    conf.setInt("heartbeat.recheck.interval", 60000);
    ...
    conf.setInt("dfs.datanode.handler.count", 50);
    conf.setInt("dfs.datanode.artificialBlockReceivedDelay",artificialBlockReceivedDelay);
    conf.setBoolean("dfs.support.append", true);
    conf.setStrings("dfs.namenode.port.list", "0");
    ...
    try {
      ...
      for (int i = 0; i < numberOfFiles; i++) {
        short replication = (short)(SNAppendTestUtil.nextInt(numDatanodes) + 1);
        // short replication = 3;
  ...
  public static void main(String []args) throws Throwable {
    ...
    TestSNFileAppend2 tfa2 = new TestSNFileAppend2();
