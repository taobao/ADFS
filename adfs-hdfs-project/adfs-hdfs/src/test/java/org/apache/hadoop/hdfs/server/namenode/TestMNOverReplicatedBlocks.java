...
public class TestMNOverReplicatedBlocks extends TestCase {
  public static final Log LOG = LogFactory
      .getLog(TestMNOverReplicatedBlocks.class);
  ...
  public void testProcesOverReplicateBlock1() throws IOException {
    ...
    internalTestProcesOverReplicateBlock(cluster);
  }
  
  //nn0 has 0 dn, nn1 has 3 dn
  public void testProcesOverReplicateBlock2() throws IOException {
    Configuration conf = new Configuration();
    conf.setLong("dfs.blockreport.intervalMsec", 1000L);
    conf.set("dfs.replication.pending.timeout.sec", Integer.toString(2));
    URL url = DFSTestUtil.class.getResource("mini-dfs-conf.xml");
  ...
  public void testProcesOverReplicateBlock3() throws IOException {
    Configuration conf = new Configuration();
    conf.setLong("dfs.blockreport.intervalMsec", 1000L);
    conf.set("dfs.replication.pending.timeout.sec", Integer.toString(2));
    URL url = DFSTestUtil.class.getResource("mini-dfs-conf.xml");
    conf.addResource(url);
    conf.set("dfs.namenode.port.list", "0,0");
    MiniMNDFSCluster cluster = new MiniMNDFSCluster(conf, 2, 0, true, null);
    cluster.startDataNodes(conf, 1, 1, true, null, null, null);
    internalTestProcesOverReplicateBlock(cluster);
  }
  
  private void internalTestProcesOverReplicateBlock(MiniMNDFSCluster cluster) throws IOException{
...
//      FSNamesystem namesystem = FSNamesystem.getFSNamesystem();
      ...
      FSNamesystem namesystem= namenodes.get(0).namesystem;
      ...
      NumberReplicas numRep = namesystem.countNodes(block);
      assertEquals(1, numRep.liveReplicas());
    } finally {
      cluster.shutdown();
    }  
  }
