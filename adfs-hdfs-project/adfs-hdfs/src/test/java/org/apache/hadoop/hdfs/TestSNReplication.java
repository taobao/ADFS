 ...
 */
public class TestSNReplication extends TestCase {
  ...
  public void runReplication(boolean simulated) throws IOException {
    ...
    conf.setStrings("dfs.namenode.port.list", "0");
    MiniMNDFSCluster cluster = new MiniMNDFSCluster(conf, numDatanodes, true, racks);
    cluster.waitActive();
  ...
  public void testPendingReplicationRetry() throws IOException {
    ...
    try {
      ...
      OutputStream out = cluster.getFileSystemCToFreeNamenode(-1).create(testPath);
      out.write(buffer);
      out.close();
