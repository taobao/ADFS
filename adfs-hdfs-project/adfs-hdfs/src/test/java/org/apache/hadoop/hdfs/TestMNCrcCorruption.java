 ...
 */
public class TestMNCrcCorruption extends TestCase {

  static private Configuration conf;
  static private MiniMNDFSCluster cluster;
  
  public TestMNCrcCorruption() {
    super();
   ...
   */
  private void thistest(DFSTestUtil util,String nnlist,int numDataNodes) throws Exception {
    short replFactor = 2;
    Random random = new Random();
    FileSystem fs = null;
    try {
      conf.setStrings("dfs.namenode.port.list",nnlist);
      ...
      //
      //Log LOG = LogFactory.getLog(TestMNCrcCorruption.class);
      // LOG.info("test.build.data is;" + conf.get("test.build.data"));
  ...
  public void testDFSMultiNNCrcCorruption()throws Exception {
...
//    thistest(util1,"0,0",3);

    //
    // specific parameters
    //
    System.out.println("TestCrcCorruption with specific parameters");
    conf.setInt("io.bytes.per.checksum", 17);
    conf.setInt("dfs.block.size", 34);
    DFSTestUtil util2 = new DFSTestUtil("TestCrcCorruption", 10, 3, 400);
    thistest(util2,"0,0",3);      
  ...
  public void testEntirelyCorruptFileOneDNAndMuNN() throws Exception {
    System.out.println("TestCrcCorruption: testEntirelyCorruptFileOneDNAndOneNN");  
    doTestEntirelyCorruptFile(1,"0");    
  }
  ...
  public void testEntirelyCorruptFileThreeDNAndMuNN() throws Exception {
    System.out.println("TestCrcCorruption: testEntirelyCorruptFileThreeDNAndOneNN");
    doTestEntirelyCorruptFile(3,"0,0");   
  }

  private void doTestEntirelyCorruptFile(int numDataNodes,String nnportlist) throws Exception {
    long fileSize = 4096;
    ...
    try {
      ...
      conf.setStrings("dfs.namenode.port.list",nnportlist);
      cluster = new MiniMNDFSCluster(conf, numDataNodes, true, null);
      cluster.waitActive();
      cluster.waitDatanodeDie();
      //int fs_nnindex = 0;
      ...
      DFSTestUtil.createFile(fs, file, fileSize, (short)numDataNodes, 12345L);
