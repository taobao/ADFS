 ...
 */
public class TestSNFileCreationClient extends junit.framework.TestCase {
  static final String DIR = "/" + TestSNFileCreationClient.class.getSimpleName() + "/";
  ...
  public void testClientTriggeredLeaseRecovery() throws Exception {
    ...
    MiniMNDFSCluster cluster = new MiniMNDFSCluster(conf, REPLICATION, true, null);
    cluster.waitActive();
    try {
      final FileSystem fs = cluster.getFileSystemCToFreeNamenode(-1);
      ...
      try {
        ...
        Thread.sleep(5000);                       // let writers get started

        //stop a datanode, it should have least recover.
        cluster.stopDataNode(SNAppendTestUtil.nextInt(REPLICATION));
