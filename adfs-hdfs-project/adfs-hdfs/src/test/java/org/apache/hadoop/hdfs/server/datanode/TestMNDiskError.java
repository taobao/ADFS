...
public class TestMNDiskError extends TestCase {
  /**
   * All datanodes connect to NN 0 and client connects to NN 1.  
   * @throws Exception
   */
  ...
  /**
   * DN 0 connects to NN 0, DN 1 connects to NN 1, and client connects to NN 1.
   * @throws Exception
   */
  public void testReplicationErrorDiffNN() throws Exception {
    // bring up a cluster of 1
    Configuration conf = new Configuration();
    URL url=DFSTestUtil.class.getResource("mini-dfs-conf.xml");
    conf.addResource(url);
    conf.set("dfs.namenode.port.list", "0,0");
    //conf.set("datanodemap.update.interval", "500");
    MiniMNDFSCluster cluster = new MiniMNDFSCluster(conf, 1, 0, true, null);
    cluster.waitActive();
    cluster.waitDatanodeDie();
    FileSystem fs = cluster.getFileSystem(1);
    ...
    try {
      ...
      LocatedBlocks blocks = cluster.getNameNode(0).namesystem.getBlockLocations(
          fileName.toString(), 0, (long)fileLen);
      assertEquals(blocks.locatedBlockCount(), 1);
      LocatedBlock block = blocks.get(0);
      
      // bring up a second datanode and connects it to NN 1
      ...
      // the following replication factor 2 is different from community 
      // version, because adfs functionality is different from HDFS 
      // in this area. 
      DFSTestUtil.waitReplication(fs, fileName, (short)2);
