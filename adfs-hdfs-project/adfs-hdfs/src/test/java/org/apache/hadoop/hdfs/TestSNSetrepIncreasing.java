...
public class TestSNSetrepIncreasing extends TestCase {
  static void setrep(int fromREP, int toREP, boolean simulatedStorage) throws IOException {
    ...
    MiniMNDFSCluster cluster = new MiniMNDFSCluster(conf, 10, true, null);
    FileSystem fs = cluster.getFileSystemCToFreeNamenode(-1);
    assertTrue("Not a HDFS: "+fs.getUri(), fs instanceof DistributedFileSystem);

    try {
      Path root = TestMNDFSShell.mkdir(fs, new Path("/test/setrep" + fromREP + "-" + toREP));
      Path f = TestMNDFSShell.writeFile(fs, new Path(root, "foo"));
      ...
      TestMNDFSShell.show("done setrep waiting: " + root);
    } finally {
