 ...
 */
public class TestMNNamenodeCapacityReport extends TestCase {
  private static final Log LOG = LogFactory.getLog(TestMNNamenodeCapacityReport.class);

  //cluster has 1 dn
  public void testVolumeSize1() throws Exception {
    ...
    LOG.info("^^^^^^^^^^"+conf);
    cluster = new MiniMNDFSCluster(conf, numOfDn, true, null);
    internalTestVolumeSize(cluster, reserved, conf, numOfDn);
  }
   
  //cluster has 3 dn
  public void testVolumeSize2() throws Exception {
    Configuration conf = new Configuration();
    MiniMNDFSCluster cluster = null;
    int numOfDn=3;
  ...
  private void internalTestVolumeSize(MiniMNDFSCluster cluster, long reserved, Configuration conf, int numOfDn) throws Exception {
	try {
	  cluster.waitActive();
	  ...
	  long diskCapacity = numOfDataDirs * df.getCapacity()*numOfDn;
