...
public class TestSNLeaseRecovery2 extends junit.framework.TestCase {
  {
  ...
  //  conf.setInt("io.bytes.per.checksum", 16);
    ...
    try {
      ...
      //create a file
      DistributedFileSystem dfs = (DistributedFileSystem)cluster.getFileSystemCToFreeNamenode(-1);
      // create a random file name
      String filestr = "/foo" + SNAppendTestUtil.nextInt();
      ...
      int size = SNAppendTestUtil.nextInt(FILE_SIZE);
      System.out.println("size=" + size);
      stm.write(buffer, 0, size);

      // sync file
      SNAppendTestUtil.LOG.info("sync");
      stm.sync();
      SNAppendTestUtil.LOG.info("leasechecker.interruptAndJoin()");
        ...
        for(int i = 0; i < 20 && !done; i++) {
          ...
          try {
            ...
            if (message.contains("file exists")) {
              SNAppendTestUtil.LOG.info("done", ioe);
              done = true;
            }
            else if (message.contains(AlreadyBeingCreatedException.class.getSimpleName())) {
              SNAppendTestUtil.LOG.info("GOOD! got " + message);
            }
            else {
              SNAppendTestUtil.LOG.warn("UNEXPECTED IOException", ioe);
            }
          }

          if (!done) {
            SNAppendTestUtil.LOG.info("sleep " + 5000 + "ms");
      ...
      SNAppendTestUtil.LOG.info("Lease for file " +  filepath + " is recovered. "
