...
import org.apache.hadoop.hdfs.server.metachecker.MetaChecker;
...
public class TestMNMetaChecker extends TestCase {
  public static final Log LOG = LogFactory.getLog(TestMNMetaChecker.class);
  private Configuration conf;
  private StateManager state;
  private long datanodeExpireInterval;

  public TestMNMetaChecker(String name) throws Exception {
    super(name);
  ...
  protected void setUp() throws Exception {
    ...
    ZKClient.getInstance(conf).delete(FSConstants.ZOOKEEPER_METACHECKER_HOME+"/token", false);
  ...
  */
  ...
  public void testAddRepl() throws Exception{    
    ...
    ToolRunner.run(conf, new MetaChecker(), null);
    assertEquals(3,state.getFileInfo("/"+fileName.getName()).replication);
