...
import org.apache.hadoop.hdfs.server.datanode.TestMNInterDatanodeProtocol;
...
public class TestMNLeaseRecovery extends junit.framework.TestCase {
  static final int BLOCK_SIZE = 1024;
  static final short REPLICATION_NUM = (short)3;

  static void checkMetaInfo(Block b, InterDatanodeProtocol idp
      ) throws IOException {
    TestMNInterDatanodeProtocol.checkMetaInfo(b, idp, null);
  }
  ...
  throws Exception {
    ...
    String namenodeStr = "0,0";
    conf.set("dfs.namenode.port.list", namenodeStr);
    conf.setLong("dfs.block.size", BLOCK_SIZE);
    conf.setBoolean("dfs.support.append", true);
    MiniMNDFSCluster cluster = null;

    try {
      cluster = new MiniMNDFSCluster(conf, 5, 0, true, null);
      cluster.waitActive();
      cluster.waitDatanodeDie();

      //create a file
      DistributedFileSystem dfs = (DistributedFileSystem)cluster.getFileSystem(1);
      String filestr = "/foo";
      ...
      LocatedBlock locatedblock = TestMNInterDatanodeProtocol.getLastLocatedBlock(
      ...
      for(int i = 0; i < REPLICATION_NUM; i++) {
        newblocksizes[i] = SNAppendTestUtil.nextInt(lastblocksize);
      }
      ...
      cluster.getNameNode(1).append(filestr, dfs.dfs.clientName);

      //block synchronization
      final int primarydatanodeindex = SNAppendTestUtil.nextInt(datanodes.length);
...
//      long currentGS = cluster.getNameNode(1).namesystem.getGenerationStamp();
