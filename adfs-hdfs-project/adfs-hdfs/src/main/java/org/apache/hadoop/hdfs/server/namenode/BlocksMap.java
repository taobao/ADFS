...
public class BlocksMap {
  ...
  public static class BlockInfo extends Block implements LightWeightGSet.LinkedElement {
    ...
    private int ensureCapacity(int num) {
      assert this.triplets != null : "BlockInfo is not initialized";
      int last = numNodes();
      if (triplets.length >= (last + num) * 3) return last;
      /*
    ...
    int numNodes() {
      ...
      for (int idx = getCapacity() - 1; idx >= 0; idx--) {
        if (getDatanode(idx) != null) return idx + 1;
      }
      return 0;
    }
    ...
    BlockInfo listInsert(BlockInfo head, DatanodeDescriptor dn) {
      ...
      if (head != null) head.setPrevious(head.findDatanode(dn), this);
      return this;
    }
  ...
  boolean contains(Block block, DatanodeDescriptor datanode) {
    ...
    if (-1 == info.findDatanode(datanode)) return false;
