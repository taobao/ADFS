...
class ReplicationTargetChooser {
  ...
  DatanodeDescriptor[] chooseTarget(int numOfReplicas, DatanodeDescriptor writer,
    ...
    if (numOfReplicas == 0 || fs.stateManager.clusterMap.getNumOfLeaves() == 0) { return new DatanodeDescriptor[0]; }

    if (excludedNodes == null) {
      excludedNodes = new ArrayList<Node>();
    }

    int clusterSize = fs.stateManager.clusterMap.getNumOfLeaves();
    ...
    int maxNodesPerRack = (totalNumOfReplicas - 1) / fs.stateManager.clusterMap.getNumOfRacks() + 2;

    List<DatanodeDescriptor> results = new ArrayList<DatanodeDescriptor>(choosenNodes);
    excludedNodes.addAll(choosenNodes);

    if (!fs.stateManager.clusterMap.contains(writer)) {
      writer = null;
    }
  ...
  private DatanodeDescriptor chooseTarget(int numOfReplicas, DatanodeDescriptor writer, List<Node> excludedNodes,
    ...
    if (numOfReplicas == 0 || fs.stateManager.clusterMap.getNumOfLeaves() == 0) { return writer; }
    ...
    try {
      ...
      case 2:
        if (fs.stateManager.clusterMap.isOnSameRack(results.get(0), results.get(1))) {
          chooseRemoteRack(1, results.get(0), excludedNodes, blocksize, maxNodesPerRack, results);
        } else if (newBlock) {
      ...
      FSNamesystem.LOG.warn("Not able to place enough replicas, still in need of " + numOfReplicas + "(excluded: "
   ...
   */
  private DatanodeDescriptor chooseLocalNode(DatanodeDescriptor localMachine, List<Node> excludedNodes, long blocksize,
   ...
   * @return the choosen node
   */
  private DatanodeDescriptor chooseLocalRack(DatanodeDescriptor localMachine, List<Node> excludedNodes, long blocksize,
      int maxNodesPerRack, List<DatanodeDescriptor> results) throws NotEnoughReplicasException {
  ...
  private DatanodeDescriptor chooseRandom(String nodes, List<Node> excludedNodes, long blocksize, int maxNodesPerRack,
    ...
    do {
      ...
      if (selectedNodes.length == 0) { throw new NotEnoughReplicasException("Not able to place enough replicas"); }
  ...
  private DatanodeDescriptor[] chooseRandom(int numOfReplicas, String nodes, List<Node> excludedNodes) {
    ...
    int numOfAvailableNodes = fs.stateManager.clusterMap.countNumOfAvailableNodes(nodes, excludedNodes);
    numOfReplicas = (numOfAvailableNodes < numOfReplicas) ? numOfAvailableNodes : numOfReplicas;
    while (numOfReplicas > 0) {
      DatanodeDescriptor choosenNode = (DatanodeDescriptor) (fs.stateManager.clusterMap.chooseRandom(nodes));
  ...
  private boolean isGoodTarget(DatanodeDescriptor node, long blockSize, int maxTargetPerLoc, boolean considerLoad,
    ...
    if (considerLoad) {
      double avgLoad = 0;
      int size = fs.stateManager.clusterMap.getNumOfLeaves();
      if (size != 0) {
  ...
  private DatanodeDescriptor[] getPipeline(DatanodeDescriptor writer, DatanodeDescriptor[] nodes) {
    ...
    synchronized (fs.stateManager.clusterMap) {
      int index = 0;
      if (writer == null || !fs.stateManager.clusterMap.contains(writer)) {
        writer = nodes[0];
      }
      for (; index < nodes.length; index++) {
        DatanodeDescriptor shortestNode = nodes[index];
        int shortestDistance = fs.stateManager.clusterMap.getDistance(writer, shortestNode);
        int shortestIndex = index;
        for (int i = index + 1; i < nodes.length; i++) {
          DatanodeDescriptor currentNode = nodes[i];
          int currentDistance = fs.stateManager.clusterMap.getDistance(writer, currentNode);
  ...
  public static int verifyBlockPlacement(LocatedBlock lBlk, int minRacks, NetworkTopology cluster) {
    ...
    if (locs == null) locs = new DatanodeInfo[0];
