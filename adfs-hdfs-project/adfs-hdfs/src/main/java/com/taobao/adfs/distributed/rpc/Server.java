...
/**
 * An abstract IPC service. IPC calls take a single {@link Writable} as a parameter, and return a {@link Writable} as
 * their value. A service runs on a port and is defined by a parameter class and a value class.
 * 
 * @see Client
 */
public abstract class Server {
   ...
   * implementations, and under {@link Writable} methods of paramters and return values. Permits applications to access
   * the server context.
   */
  ...
  public static InetAddress getRemoteIp() {
    ...
    if (call != null) { return call.connection.socket.getInetAddress(); }
    return null;
  }
  ...
  protected long rpcQueueTime = 0;
  protected long rpcProcessingTime = 0;
  ...
  private static class Call {
    ...
    public Call(Configuration conf, Class<? extends Writable> paramClass, ByteBuffer byteBuffer, Connection connection) {
      this.conf = conf;
    ...
    public String toString() {
      return connection.toString() + "->" + param;
    }
  ...
  private class Listener extends Thread {
    ...
    public Listener() throws IOException {
      ...
      // create a selector for listener thread to accept connection;
      selector = Selector.open();
      readers = new Reader[readThreads];
      readPool = Executors.newFixedThreadPool(readThreads);
      for (int i = 0; i < readThreads; i++) {
        // comments by jiwan@taobao.com: open a selector for every reader to read data
    ...
    private class Reader implements Runnable {
      ...
      /**
       * This gets reader into the state that waits for the new channel to be registered with readSelector. If it was
       * waiting in select() the thread will be woken up, otherwise whenever select() is called it will return even if
       * there is nothing to read and wait in while(adding) for finishAdd call
       */
    ...
    /**
     * cleanup connections from connectionList. Choose a random range to scan and also have a limit on the number of the
     * connections that will be cleanedup per run. The criteria for cleanup is the time for which the connection was
     * idle. If 'force' is true then all connections will be looked at for the cleanup.
     */
    private void cleanupConnections(boolean force) {
      if (force || numConnections > thresholdIdleConnections) {
        long currentTime = System.currentTimeMillis();
        if (!force && (currentTime - lastCleanupRunTime) < cleanupInterval) { return; }
        int start = 0;
    ...
    void doAccept(SelectionKey key) throws IOException, OutOfMemoryError {
      ...
      while ((channel = server.accept()) != null) {
        ...
        // comments by jiwan@taobao.com: choose a reader to read the data
    ...
    void doRead(SelectionKey key) throws InterruptedException {
      ...
      if (count < 0) {
        if (LOG.isDebugEnabled())
          LOG.debug(getName() + ": disconnecting client " + c.getHostAddress() + ". Number of active connections: "
              + numConnections);
        closeConnection(c);
        c = null;
      } else {
    ...
    synchronized void doStop() {
      ...
      if (readPool != null) {
        try {
          ...
          LOG.info(getName() + ":Exception in shutdown read pool. " + e);
        }
      }
    }
      ...
      synchronized (call.connection.responseQueue) {
        if (processResponse(call.connection.responseQueue, false)) {
          try {
            ...
            /*
             * The Listener/reader might have closed the socket. We don't explicitly cancel the key, so not sure if this
      ...
      try {
        synchronized (responseQueue) {
          ...
          if (!call.response.hasRemaining()) {
            ...
            if (LOG.isDebugEnabled()) {
              LOG.debug(getName() + ": responding to #" + call.id + " from " + call.connection + " Wrote partial "
                  + numBytes + " bytes.");
            }
          }
  ...
  private class Connection {
    ...
    public int readAndProcess() throws IOException, InterruptedException {
      while (true) {
        /*
         * Read at most one RPC. If the header is not read completely yet then iterate until we read first RPC or until
        ...
        if (!versionRead) {
          ...
          if (!HEADER.equals(dataLengthBuffer) || version != CURRENT_VERSION) {
            ...
            LOG.warn("Incorrect header or version mismatch from " + hostAddress + ":" + remotePort + " got version "
        ...
        if (data == null) {
          dataLengthBuffer.flip();
          // comments by jiwan@taobao.com: client sends length before sending real data
          ...
          // comments by jiwan@taobao.com:data is complete call/header
        ...
        // comments by jiwan@taobao.com: when data is complete data.remaining() will be 0
        if (data.remaining() == 0) {
          dataLengthBuffer.clear();
          data.flip();
          if (headerRead) {
    ...
    /**
     * comments by jiwan@taobao.com: when processData is called, the data is a complete call
     */
    private void processData() throws IOException, InterruptedException {
      Call call = new Call(conf, paramClass, data, this);
      ...
      while (running) {
        try {
          ...
          try {
            ...
            if (!ObjectWritable.isWritable(value))
              throw new IOException("return value is not writable, returnValueToString=" + value);
          } catch (Throwable e) {
          ...
          // write result length with 0
          out.writeInt(0);
          int resultOffset = buf.size();
          ...
          // write result length with real length
          int resultLength = buf.size() - resultOffset;
          byte[] responseBytes = buf.toByteArray();
          responseBytes[resultOffset - 4] = (byte) ((resultLength) >>> 24 & 0xFF);
          responseBytes[resultOffset - 3] = (byte) ((resultLength) >>> 16 & 0xFF);
          responseBytes[resultOffset - 2] = (byte) ((resultLength) >>> 8 & 0xFF);
          responseBytes[resultOffset - 1] = (byte) ((resultLength) >>> 0 & 0xFF);
          call.setResponse(ByteBuffer.wrap(responseBytes));
  ...
  public long getCallQueueTime() {
    return rpcQueueTime;
  }

  public long setCallQueueTime(long rpcQueueTime) {
    return this.rpcQueueTime;
  }

  public long getRpcProcessingTime() {
    return rpcProcessingTime;
  }

  public long setRpcProcessingTime(long rpcProcessingTime) {
    return this.rpcProcessingTime;
  }

  /**
   * When the read or write buffer size is larger than this limit, i/o will be done in chunks of this size. Most RPC
   * requests and responses would be be smaller.
   */
   ...
   * {@link #channelWrite(WritableByteChannel, ByteBuffer)}. Only one of readCh or writeCh should be non-null.
   * 
