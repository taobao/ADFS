...
public class Trash extends Configured {
  ...
  public boolean moveToTrash(Path path) throws IOException {
    ...
    if (qpath.startsWith(trash.toString())) { return false; // already in trash
    }

    if (trash.getParent().toString().startsWith(qpath)) { throw new IOException("Cannot move \"" + path
          ...
          try {
            ...
            if (now >= end) {
              ...
              for (FileStatus home : homes) { // dump each trash
                ...
                Trash trash = null;
                try {
                  trash = new Trash(home.getPath(), conf);
                  trash.expunge();
                  trash.checkpoint();
                } catch (IOException e) {
                  LOG.warn("Trash caught: " + e + ". Skipping " + home.getPath() + ".");
                } finally {
                  if (trash != null) trash.close();
                }
              }
            }
          } catch (Exception e) {
  ...
  public static void main(String[] args) throws Exception {
    Trash.getEmptier(new Configuration()).run();
  }
}
